...
churn<-as.numeric(NN_wdata$churn)-1
NN_data<-cbind(churn,
               subset(NN_wdata,select=c(eqpdays, months, avg3mou, avg3qty, 
...
nnl5 <- neuralnet(f,data=train_,hidden=5,stepmax=1e+07,linear.output=T)
...
#change nn
pr.nnl63 <- compute(nnl63,test_[,-1])
pr.nnl63_ <- (pr.nnl63$net.result - min(pr.nnl63$net.result))/(max(pr.nnl63$net.result)-min(pr.nnl63$net.result))
MSE.nnl63 <- sum((test.r - pr.nnl63_)^2)/nrow(test_)
